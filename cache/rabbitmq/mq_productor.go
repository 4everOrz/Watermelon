...
func init() {
	...
	MQueueName = config.GetString("RabbitMQ_QueueName")
	MExchange = config.GetString("RabbitMQ_Exchange")
	MExchangeType = config.GetString("RabbitMQ_ExchangeType")
	MMqurl = config.GetString("RabbitMQ_Mqurl")
	MKey = config.GetString("RabbitMQ_Key")
}
...
func (c *MqProducer) rabbitConnector() {
	...
	for {
		...
		if rabbitErr != nil {
			...
			log.Println("mqproducer connect error,try reconnect", rabbitErr)
			c.conn = nil
			c.channel = nil
			c.connectToRabbitMQ()
			c.channel, _ = c.conn.Channel()
			ProducerConnTag = true
			log.Println("mqproducer connect ok!")
